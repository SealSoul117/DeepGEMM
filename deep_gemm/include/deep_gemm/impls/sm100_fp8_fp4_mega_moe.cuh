...
sm100_fp8_fp4_mega_moe_impl(void* y,
                            ...
                            const __grid_constant__ trace::TraceBuf trace_buf) {
    ...
    // === [TRACE v2] Initialize per-SM trace header (records globaltimer + clock baseline) ===
    // This must run once per SM. Only thread 0 writes; the rest of the block waits on syncthreads.
    if (thread_idx == 0) {
        trace::trace_sm_init(trace_buf);
    }
    __syncthreads();
    // === [/TRACE v2] =======================================================================
    ...
    if (warp_idx < kNumDispatchWarps) {
        ...
        // === [TRACE v2] Per-warp register counter for dispatch role ===
        // This counter lives in a register, NOT shared memory, NOT atomic.
        // It indexes into trace_buf's per-(SM, role=DISPATCH) sub-region.
        uint32_t dispatch_evt_idx = 0;
        trace::trace_event(trace_buf, trace::WR_DISPATCH, dispatch_evt_idx,
                           trace::EV_DISPATCH_START, warp_idx);
        // === [/TRACE v2] ==============================================

        // TRACE_BEGIN(t_dispatch);
        // uint64_t t_dispatch = deep_gemm::trace::now_ns();
        ...
        // if (cute::elect_one_sync())                                   
        //     deep_gemm::trace::record(trace_buf, deep_gemm::trace::WR_DISPATCH, deep_gemm::trace::EV_DISPATCH_PHASE, t_dispatch, deep_gemm::trace::now_ns(), 0xFFFFFFFFu, warp_idx);              

        // === [TRACE v2] End of dispatch phase on this warp ===
        trace::trace_event(trace_buf, trace::WR_DISPATCH, dispatch_evt_idx,
                           trace::EV_DISPATCH_END, warp_idx);
        // === [/TRACE v2] =====================================
        ...
        // === [TRACE v2] Per-warp register counter for TMA-A role ===
        uint32_t tma_a_evt_idx = 0;
        // === [/TRACE v2] ===========================================
        scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
            ...
            // === [TRACE v2] Mark start of this expert on TMA-A ===
            // aux16 = (wave_idx << 8) | local_expert_idx; both small ints, fits in 16 bits
            const uint16_t expert_aux = static_cast<uint16_t>((scheduler.wave_idx << 8) | (local_expert_idx & 0xff));
            trace::trace_event(trace_buf, trace::WR_TMA_LOAD_A, tma_a_evt_idx,
            ...
            if (block_phase == sched::BlockPhase::Linear1) {
                // TRACE_BEGIN(t_wait);
                /* ========== PRODUCTION CODE — DO NOT REMOVE OR COMMENT THESE 3 LINES ========== */
                const auto ptr = workspace.get_l1_arrival_count_ptr(pool_block_idx);
                const auto expected = scheduler.template get_valid_m<false>();
                while (ptx::ld_acq(ptr) != expected);
                /* ============================================================================= */
                // === [TRACE v2] Wait completed; gap from previous event = wait duration ===
                trace::trace_event(trace_buf, trace::WR_TMA_LOAD_A, tma_a_evt_idx,
                                   trace::EV_AFTER_L1_ARRIVAL, expert_aux);
                // === [/TRACE v2] ==========================================================
                ...
                /* ============================================================================= */
                // === [TRACE v2] Wait completed ===
                trace::trace_event(trace_buf, trace::WR_TMA_LOAD_A, tma_a_evt_idx,
                                   trace::EV_AFTER_L2_ARRIVAL, expert_aux);
                // === [/TRACE v2] =================
                // TRACE_END(trace_buf, deep_gemm::trace::WR_TMA_LOAD_A, deep_gemm::trace::EV_L2_ARRIVAL_WAIT, t_wait, wave, local_expert_idx);
            }
            
            // NOTE: deliberately NOT tracing inside the k-block loop below. With ~40 k-blocks per
            // expert, tracing here would flood the buffer and might cause SMEM pressure.
            // TRACE_BEGIN(t_expert);
            for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                // Wait consumer release
                // TRACE_BEGIN(t_eb);
                /* ========== PRODUCTION CODE — DO NOT REMOVE ========== */
            ...
            // === [TRACE v2] Mark end of this expert on TMA-A ===
            trace::trace_event(trace_buf, trace::WR_TMA_LOAD_A, tma_a_evt_idx,
                               block_phase == sched::BlockPhase::Linear1 ? trace::EV_WAVE_L1_END : trace::EV_WAVE_L2_END,
                               expert_aux);
            // === [/TRACE v2] ===================================
        ...
        if (is_leader_cta) {
            ...
            scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
                ...
                const uint16_t expert_aux = static_cast<uint16_t>((wave << 8) | (local_expert_idx & 0xff));

                // === [TRACE v2] Mark start of expert on MMA ===
                trace::trace_event(trace_buf, trace::WR_MMA, mma_evt_idx,
                                   block_phase == sched::BlockPhase::Linear1 ? trace::EV_WAVE_L1_START : trace::EV_WAVE_L2_START,
                                   expert_aux);
                // === [/TRACE v2] ==============================
                ...
                /* This is the tmem-accumulator-slot empty wait. Removing it      */
                /* causes deadlock: MMA overwrites tmem while epilogue is reading.*/
                tmem_empty_barriers[accum_stage_idx]->wait(accum_phase ^ 1);
                /* =============================================================== */
                // === [TRACE v2] Wait completed; gap from prev event = tmem-empty wait time ===
                trace::trace_event(trace_buf, trace::WR_MMA, mma_evt_idx,
                                   trace::EV_AFTER_TMEM_EMPTY, expert_aux);
                // === [/TRACE v2] =============================================================
                ...
                // === [TRACE v2] Mark end of expert on MMA ===
                trace::trace_event(trace_buf, trace::WR_MMA, mma_evt_idx,
                                   block_phase == sched::BlockPhase::Linear1 ? trace::EV_WAVE_L1_END : trace::EV_WAVE_L2_END,
                                   expert_aux);
                // === [/TRACE v2] ============================
        ...
        // Persistently schedule over blocks
        uint32_t current_iter_idx = 0;
        // === [TRACE v2] Per-warp register counter for EPILOGUE role ===
        uint32_t epi_evt_idx = 0;
        // === [/TRACE v2] ==============================================
        scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
                                     const uint32_t& local_expert_idx,
                                     const uint32_t& num_k_blocks,
            ...
            // === [TRACE v2] Mark start of expert on EPILOGUE ===
            trace::trace_event(trace_buf, trace::WR_EPILOGUE, epi_evt_idx,
                               block_phase == sched::BlockPhase::Linear1 ? trace::EV_WAVE_L1_START : trace::EV_WAVE_L2_START,
                               expert_aux);
            // === [/TRACE v2] ===================================
            ...
            /* ========== PRODUCTION CODE — DO NOT REMOVE THIS LINE ========== */
            /* Wait for MMA to fill tmem accumulator slot.                    */
            tmem_full_barriers[accum_stage_idx]->wait(accum_phase);
            /* =============================================================== */
            // === [TRACE v2] Wait completed; gap from prev event = tmem-full wait time ===
            trace::trace_event(trace_buf, trace::WR_EPILOGUE, epi_evt_idx,
                               trace::EV_AFTER_TMEM_FULL, expert_aux);
            // === [/TRACE v2] ============================================================
            ...
            // === [TRACE v2] Mark end of expert on EPILOGUE ===
            trace::trace_event(trace_buf, trace::WR_EPILOGUE, epi_evt_idx,
                               block_phase == sched::BlockPhase::Linear1 ? trace::EV_WAVE_L1_END : trace::EV_WAVE_L2_END,
                               expert_aux);
            // === [/TRACE v2] =================================
        ...
        // === [TRACE v2] All MoE epilogues done on this warp; combine phase begins ===
        // Combine uses a separate counter because it lives in the same epilogue warp
        // but is a logically distinct phase.
        uint32_t combine_evt_idx = 0;
        trace::trace_event(trace_buf, trace::WR_COMBINE, combine_evt_idx,
                           trace::EV_COMBINE_START, epilogue_warp_idx);
        // === [/TRACE v2] ============================================================
        ...
        // === [TRACE v2] Mark end of combine on this warp ===
        trace::trace_event(trace_buf, trace::WR_COMBINE, combine_evt_idx,
                           trace::EV_COMBINE_END, epilogue_warp_idx);
        // === [/TRACE v2] ===================================
    }
    trace_sm_finalize(trace_buf);
#else
